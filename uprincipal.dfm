object Frmprincipal: TFrmprincipal
  Left = 0
  Top = 0
  Caption = 'USAR COMBOBOX Vers'#227'o 1.3'
  ClientHeight = 363
  ClientWidth = 605
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poDesktopCenter
  OnKeyPress = FormKeyPress
  PixelsPerInch = 96
  TextHeight = 13
  object PageControl1: TPageControl
    Left = 0
    Top = 41
    Width = 605
    Height = 281
    ActivePage = TabSheet2
    Align = alClient
    TabOrder = 0
    object TabSheet1: TTabSheet
      Caption = '&Cadastrar - F2'
      ExplicitTop = 23
      object Label5: TLabel
        Left = 136
        Top = 24
        Width = 335
        Height = 23
        Caption = 'MANIPUNALDO DADOS NOS EDTIS'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlue
        Font.Height = -19
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label6: TLabel
        Left = 48
        Top = 91
        Width = 50
        Height = 13
        Caption = 'C'#243'digo .: '
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label7: TLabel
        Left = 48
        Top = 139
        Width = 44
        Height = 13
        Caption = 'Nome .: '
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object lblsubhgrupo: TLabel
        Left = 48
        Top = 187
        Width = 67
        Height = 13
        Caption = 'SubGrupo .: '
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object EdtCodigo: TEdit
        Left = 120
        Top = 88
        Width = 121
        Height = 21
        CharCase = ecUpperCase
        Enabled = False
        TabOrder = 0
      end
      object EdtNome: TEdit
        Left = 120
        Top = 133
        Width = 417
        Height = 21
        CharCase = ecUpperCase
        TabOrder = 1
      end
      object EdtSubGrupo: TEdit
        Left = 120
        Top = 184
        Width = 57
        Height = 21
        TabOrder = 2
        OnExit = EdtSubGrupoExit
      end
      object BtnSubGrupo: TBitBtn
        Left = 182
        Top = 184
        Width = 25
        Height = 21
        Caption = '...'
        TabOrder = 3
      end
      object EdTDescricao: TEdit
        Left = 212
        Top = 184
        Width = 315
        Height = 21
        BevelInner = bvNone
        BevelKind = bkFlat
        BevelOuter = bvNone
        BorderStyle = bsNone
        Color = clInfoBk
        TabOrder = 4
      end
    end
    object TabSheet2: TTabSheet
      Caption = '&Buscar - F3'
      ImageIndex = 1
      object Label1: TLabel
        Left = 5
        Top = 9
        Width = 82
        Height = 13
        Caption = 'Op'#231#245'es de Busca'
      end
      object Label2: TLabel
        Left = 154
        Top = 9
        Width = 76
        Height = 13
        Caption = 'Dados da Busca'
      end
      object cbb1: TComboBox
        Left = 3
        Top = 28
        Width = 145
        Height = 21
        TabOrder = 0
        Items.Strings = (
          '1 - Por C'#243'digo'
          '2 - Por Nome'
          '3 - Todos')
      end
      object edtPesquisa: TEdit
        Left = 154
        Top = 28
        Width = 303
        Height = 21
        CharCase = ecUpperCase
        TabOrder = 1
      end
      object BitBtn1: TBitBtn
        Left = 463
        Top = 12
        Width = 118
        Height = 40
        Caption = '&Buscar'
        TabOrder = 2
        OnClick = BitBtn1Click
      end
      object DBGrid1: TDBGrid
        Left = 0
        Top = 58
        Width = 597
        Height = 195
        Align = alBottom
        DataSource = DataSource1
        Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgTabs, dgMultiSelect, dgTitleClick, dgTitleHotTrack]
        TabOrder = 3
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        OnDblClick = DBGrid1DblClick
      end
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 322
    Width = 605
    Height = 41
    Align = alBottom
    TabOrder = 1
    object BtnSalvar: TBitBtn
      Left = 4
      Top = 0
      Width = 101
      Height = 39
      Caption = '&Salvar'
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      TabOrder = 0
      OnClick = BtnSalvarClick
    end
    object BitBtn2: TBitBtn
      Left = 106
      Top = 0
      Width = 101
      Height = 39
      Caption = '&Alterar'
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      TabOrder = 1
      OnClick = BitBtn2Click
    end
    object BtnExcluir: TBitBtn
      Left = 208
      Top = 0
      Width = 101
      Height = 39
      Caption = '&Excluir'
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      TabOrder = 2
      OnClick = BtnExcluirClick
    end
    object BtnCancelar: TBitBtn
      Left = 310
      Top = 0
      Width = 101
      Height = 39
      Caption = '&Cancelar'
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      TabOrder = 3
    end
    object BitBtn3: TBitBtn
      Left = 503
      Top = 1
      Width = 101
      Height = 39
      Align = alRight
      Caption = '&Fechar'
      Glyph.Data = {
        
        3F94003F94003F94003F94003F94003F94FFFFFFFFFFFFFFFFFF}
      TabOrder = 4
      OnClick = BitBtn3Click
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 0
    Width = 605
    Height = 41
    Align = alTop
    Caption = 'USO DO COMBOBOX'
    Ctl3D = True
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlue
    Font.Height = -21
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentCtl3D = False
    ParentFont = False
    TabOrder = 2
    object Label3: TLabel
      Left = 577
      Top = 14
      Width = 4
      Height = 16
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clRed
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label4: TLabel
      Left = 528
      Top = 14
      Width = 38
      Height = 16
      Caption = 'Reg.: '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clRed
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
  end
  object FDConnection1: TFDConnection
    Params.Strings = (
      
        'Database=C:\Users\Familia\Documents\Projetos\Projeto_Pesquisa\Ba' +
        'nco\BANCO.FDB'
      'User_Name=SYSDBA'
      'Password=masterkey'
      'Protocol=TCPIP'
      'Server=LOCALHOST'
      'Port=3050'
      'DriverID=FB')
    Connected = True
    LoginPrompt = False
    Left = 360
    Top = 8
  end
  object FDQuery1: TFDQuery
    Connection = FDConnection1
    SQL.Strings = (
      'select * from grupo')
    Left = 416
    Top = 8
  end
  object DataSource1: TDataSource
    DataSet = FDQuery1
    Left = 296
    Top = 8
  end
  object FDPhysFBDriverLink1: TFDPhysFBDriverLink
    VendorLib = 
      'C:\Users\Familia\Documents\Projetos\Projeto_Pesquisa\Dll\fbclien' +
      't.dll'
    Left = 480
    Top = 8
  end
  object zQry: TFDQuery
    Connection = FDConnection1
    SQL.Strings = (
      'select * from grupo')
    Left = 232
    Top = 8
    object zQryCODIGO: TIntegerField
      FieldName = 'CODIGO'
      Origin = 'CODIGO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      ReadOnly = True
    end
    object zQryNOME: TStringField
      FieldName = 'NOME'
      Origin = 'NOME'
      Size = 30
    end
  end
  object DSdados: TDataSource
    DataSet = zQry
    Left = 188
    Top = 9
  end
  object Qry: TFDQuery
    Connection = FDConnection1
    SQL.Strings = (
      'Select * from SUBGRUPO')
    Left = 40
    Top = 16
  end
end
